...
def load_routing_defaults(path: Path) -> RoutingDefaults:
    ...
    if not isinstance(fitness_raw, dict) or not isinstance(cost_raw, dict):
        raise RoutingDefaultsError(f"{path}: 'fitness' and 'cost' must be mappings.")
    ...
    return RoutingDefaults(fitness=fitness, cost=cost)
